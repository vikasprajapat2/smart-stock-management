...
export const PurchaseOrdersView: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      if (supData.length > 0 && !editingPoId) setSelectedSupplierId(supData[0].id.toString());
      if (whData.length > 0 && !editingPoId) setSelectedWhId(whData[0].id.toString());
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const closeBuilder = () => {
    ...
    if (warehouses.length > 0) setSelectedWhId(warehouses[0].id.toString());
  };

  const startEditPO = (po: PurchaseOrderResponse) => {
    setEditingPoId(po.id);
    setSelectedSupplierId(po.supplier_id.toString());
    ...
    try {
      if (editingPoId !== null) {
        await updatePurchaseOrder(
          editingPoId,
          undefined,
          selectedWhId ? parseInt(selectedWhId) : undefined,
          parseInt(selectedSupplierId),
          poItems
        );
        setSuccessMsg('Purchase Order successfully updated.');
      } else {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {showBuilder && (
        <form onSubmit={handleSubmitPO} className="glass-panel" style={{ padding: '1.5rem', background: 'rgba(255,255,255,0.02)', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#fff', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Layers size={18} style={{ color: 'var(--accent-purple)' }} />
            {editingPoId !== null ? `Edit Purchase Order #${editingPoId}` : 'Procurement PO Compiler'}
          </h3>
          ...
          >
            {builderLoading ? <Loader className="spin-anim" size={18} /> : (editingPoId !== null ? 'Save Changes' : 'Compile & Save Purchase Order')}
          </button>
        </form>
      )}
              ...
              <div key={po.id} className="glass-panel" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem', background: 'rgba(255,255,255,0.015)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.04)', paddingBottom: '0.75rem', gap: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    {/* Download PDF button */}
                    <button
                      onClick={() => window.open(`${API_BASE}/purchase-orders/${po.id}/pdf`, '_blank')}
                      className="scan-action-btn btn-secondary"
