...
import { Database, Plus, Search, Loader, AlertTriangle, Check, RefreshCw, Trash2, Edit2, ShieldAlert, Filter } from 'lucide-react';
import { fetchInventoryRecords, createInventoryRecord, updateInventoryRecord, deleteInventoryRecord, fetchProducts, fetchWarehouses, fetchCategories, checkBackendConnection } from '../utils/api';
import type { Product, Warehouse, Inventory, Category, InventoryFilters } from '../utils/api';
...
export const InventoryRecordsView: React.FC = () => {
  ...
  const buildFilters = useCallback((): InventoryFilters => {
    const filters: InventoryFilters = {};
    if (filterCategoryId) filters.category_id = parseInt(filterCategoryId);
    if (filterWarehouseId) filters.warehouse_id = parseInt(filterWarehouseId);
    if (filterStockStatus) filters.stock_status = filterStockStatus as InventoryFilters['stock_status'];
    if (searchTerm.trim()) filters.search = searchTerm.trim();
    return filters;
  }, [filterCategoryId, filterWarehouseId, filterStockStatus, searchTerm]);

  const loadInventory = useCallback(async (silent = false) => {
    if (!silent) setLoading(true);
    setError('');
    try {
      const invData = await fetchInventoryRecords(buildFilters());
  ...
  const loadReferenceData = async () => {
    try {
      ...
      const [prodData, whData, catData] = await Promise.all([
        fetchProducts(),
        fetchWarehouses(),
        fetchCategories()
      ]);
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      ...
      const [invData, prodData, whData, catData] = await Promise.all([
        fetchInventoryRecords(buildFilters()),
        fetchProducts(),
        fetchWarehouses(),
        fetchCategories()
      ]);
  ...
  // Reload inventory when dropdown filters change (not search — that uses debounce)
  useEffect(() => {
    if (isConnected) {
      loadInventory(true);
    }
  }, [filterCategoryId, filterWarehouseId, filterStockStatus]);
  ...
  const hasActiveFilters = searchTerm || filterCategoryId || filterWarehouseId || filterStockStatus;

  // Helper to determine stock status badge
  const getStockBadge = (inv: Inventory) => {
    const p = products.find(prod => prod.id === inv.product_id);
    if (inv.quantity <= 0) {
      return { label: 'Out of Stock', color: '#ef4444', bg: 'rgba(239, 68, 68, 0.12)' };
    }
    if (p && p.reorder_level !== undefined && p.reorder_level !== null && inv.quantity <= p.reorder_level) {
      return { label: 'Low Stock', color: '#f59e0b', bg: 'rgba(245, 158, 11, 0.12)' };
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600, minWidth: '60px' }}>
        ...
        <div style={{ flex: '1 1 180px', minWidth: '150px' }}>
          <label style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.25rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Category</label>
          <select
        ...
        {/* Warehouse Filter */}
        <div style={{ flex: '1 1 180px', minWidth: '150px' }}>
          <label style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.25rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Warehouse</label>
          <select
            className="camera-select"
            style={filterSelectStyle}
        ...
        {/* Stock Status Filter */}
        <div style={{ flex: '1 1 180px', minWidth: '150px' }}>
          <label style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.25rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Stock Status</label>
          <select
            className="camera-select"
            style={filterSelectStyle}
      ...
      {showAddForm && (
        <form onSubmit={handleSave} className="glass-panel" style={{ padding: '1.5rem', background: 'rgba(0, 0, 0, 0.02)', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      {loading && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 0', gap: '1rem' }}>
          ...
          <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>Loading inventory records...</span>
        </div>
      )}
        ...
        <div className="glass-panel" style={{ padding: '0', overflowX: 'auto' }}>
          {inventory.length === 0 ? (
            <div style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <h4 style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: '0.25rem' }}>No Records Found</h4>
              <p style={{ fontSize: '0.85rem' }}>
                {hasActiveFilters ? 'No inventory records match the current filters. Try adjusting your criteria.' : 'Create a record to get started.'}
              </p>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
              ...
              <tbody>
                {inventory.map(inv => {
                  ...
                  const badge = getStockBadge(inv);
                  return (
                    <tr key={inv.id} className="table-row-hover" style={{ borderBottom: '1px solid rgba(0,0,0,0.04)', transition: 'var(--transition-smooth)' }}>
                      <td style={{ padding: '1rem', color: 'var(--text-secondary)' }}>#{inv.id}</td>
                      <td style={{ padding: '1rem' }}>
                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{p ? p.product_name : `Product ID: ${inv.product_id}`}</div>
                        {p && (
